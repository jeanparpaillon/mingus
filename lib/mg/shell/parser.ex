defmodule Mg.Shell.Parser do
  ...
  defp list(subject, _s) do
    ...
    msg =
      Enum.reduce(Store.lookup(category: category), msg, fn entity, acc ->
        id = :io_lib.format("~-40s", [Core.Entity.get(entity, :id)])

        desc =
          case Core.Entity.get(entity, :title) do
            nil -> ""
            title -> "(#{title})"
          end
  ...
  defp ask(spec, expand \\ nil) do
    ...
    desc =
      Keyword.get_lazy(spec, :description, fn ->
    ...
    prompt_spec =
      case prompt_type(Keyword.get(spec, :check)) do
        nil -> prompt_spec
  ...
  defp help(nil) do
    ...
    msg =
      Enum.reduce(Subject.all(), msg, fn {name, subject}, acc ->
  ...
  defp help(subject) do
    msg =
      Enum.reduce(Subject.actions(subject), "", fn
