defmodule Mg.Providers.Ovh do
  ...
  """
  def req(name, path, method \\ :get, body \\ nil), do: GenServer.call(name, {:req, path, method, body})

  @doc """
  Request authentication token
  """
  @spec get_token(name :: atom, rules :: :readonly | :admin, redirect_url :: String.t) :: {:ok, { validation_url :: String.t, customer_token :: String.t}} | {:error, term}
  ...
  def handle_info(:inventory, %{ token: token }=s) do
    with {:ok, {200, hosts}} <- __req__(@api_servers, token)
      do
  ...
  def __authorize__(rules, redirect_url) do
    ...
    ct = 'application/json'
    with {:ok, {{_, 200, _}, _, ans_body}} <- :httpc.request(:post, {url, hdrs, ct, body}, [], []),
         {:ok, res} <- Poison.decode(ans_body)
	    do
  ...
  def __req__(path, ck, method \\ :get, body \\ nil) do
    ...
    sig = "$1$" <>
      Base.encode16(:crypto.hash(:sha, "#{@app.as}+#{ck}+#{method_up}+#{url}+#{body}+#{ts}"), case: :lower)
    headers = [
